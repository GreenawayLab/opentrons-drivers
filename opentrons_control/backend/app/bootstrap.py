...
class OTBootstrap:
    ...
    def start_agent(self) -> None:
        ...
        The agent reads its boot-time configuration from ``postbox/`` and
        labware from ``plates/``, both relative to cwd, so the cd is
        mandatory. ``opentrons_execute`` is invoked with the absolute path
        to ``agent_main.py``; it is not a ``python -m`` runner.
