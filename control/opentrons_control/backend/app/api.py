...
def create_app(robots: Mapping[str, Robot]) -> FastAPI:
    ...
    async def open_run(
        ...
        register(executor)
        log_event(
            kind="launch", status="booking", source="manual",
            user_id=user.id, actor=user.name, robot_id=req.robot_id,
            plan_id=plan["id"], plan_name=plan["name"], config_id=plan["config_id"],
            run_id=run_id,
        )
    ...
    async def calibrate_run(
        ...
        """Run one calibration action against the open agent, before /start.

        Only valid while the run is ready (session open, not driving), and only
        for the whitelisted calibration actions, so this cannot post protocol
        commands out of band.
        """
        ex = _run_or_404(run_id)
        if ex.run.status != "ready":
            raise HTTPException(status_code=409, detail="calibration is only available before the run starts")
        if req.action not in _CALIBRATION_ACTIONS:
            raise HTTPException(status_code=400, detail=f"not a calibration action: {req.action}")
        return await ex.one_shot(req.action, req.payload)
